...
try:
    ...
    # Get the measured time components using the "Avg" names (excluding Host Init for plotting)
    kernel_time = df['Avg Kernel Time (ms)']
    host_alloc_time = df['Avg Host Alloc Time (ms)']
    device_alloc_time = df['Avg Device Alloc Time (ms)']
    # host_init_time = df['Avg Host Init Time (ms)'] # Removed reading Host Init
    ...
    # (Excluding cleanup time AND host init time as requested for the plot)
    sum_of_plotted_parts = (host_alloc_time + device_alloc_time +
                            h2d_time + kernel_time + d2h_time)

    # Calculate the overhead time (remaining difference from total).
    # This overhead now implicitly includes the average cleanup time AND host init time.
    ...
    # (Excluding Cleanup AND Host Init from the plot)
    components = {
        'Overhead': overhead_time,      # Bottom (Includes cleanup, host init, other)
    ...
    expected_cols = ['Vector Size (log2 N)', 'Avg Kernel Time (ms)', 'Avg Host Alloc Time (ms)',
                     'Avg Device Alloc Time (ms)', 'Avg H2D Time (ms)', # Removed Host Init
                     'Avg D2H Time (ms)', 'Avg Total Time (ms)'] # Removed Cleanup
    col_name = str(e).strip("'")
    # Ignore error only if it's about the cleanup or host init columns which we don't strictly need here
    if col_name not in expected_cols and col_name not in ['Avg Cleanup Time (ms)', 'Avg Host Init Time (ms)']:
...
plt.title('SAXPY Avg Execution Time Breakdown vs. Vector Size (No Host Init)') # Updated Title
...
print(f"Filtered (log2 N = 15-25) stacked bar chart (no host init) generated from '{csv_filename}'.")
