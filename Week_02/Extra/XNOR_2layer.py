...
if __name__ == "__main__":
    # 1. Define XNOR function data
    X_XNOR = np.array([[0, 0], [0, 1], [1, 0], [1, 1]])
    y_XNOR = np.array([1, 0, 0, 1])

    # 2. Configure and Create the MLP instance
    print("\n" + "="*30)
    print("Training MLP on XNOR Gate (Using BCE Gradients) - Attempt 2")
    print("="*30)

    input_size = X_XNOR.shape[1]
    # --- Hyperparameter Changes ---
    hidden_size = 4 # TRYING 2 hidden neurons instead of 4
    output_size = 1
    ...
    mlp_XNOR = MLP(n_input=input_size,
    ...
    # 3. Train the network
    mlp_XNOR.fit(X_XNOR, y_XNOR, plot_interval=plot_save_interval)

    # 4. Save the MLP OUTPUT (Prediction) REGIONS learning process as a GIF
    save_mlp_output_regions_gif(mlp_XNOR, X_XNOR, y_XNOR,
                                filename="XNOR_mlp_pred_regions_bce_tuned.gif", # New filename
                                duration_per_frame=0.1)

    # 5. Test final predictions
    print("\n--- Final Predictions for XNOR Gate ---")
    all_correct = True
    for xi, target in zip(X_XNOR, y_XNOR):
        xi_reshaped = xi.reshape(1, -1)
        prediction = mlp_XNOR.predict(xi_reshaped)[0]
    ...
    if all_correct: print("\nSUCCESS: MLP correctly learned the XNOR function!")
    ...
    if PLOT_ENABLED and mlp_XNOR.loss_history_:
        plt.figure()
        plot_step = max(1, len(mlp_XNOR.loss_history_) // 1000)
        plt.plot(range(1, len(mlp_XNOR.loss_history_) + 1, plot_step), mlp_XNOR.loss_history_[::plot_step])
        plt.xlabel(f'Epochs (x{plot_step})'); plt.ylabel('Mean Squared Error Loss')
        plt.title('MLP Training Loss Curve for XNOR (Using BCE Gradients)'); plt.grid(True)
