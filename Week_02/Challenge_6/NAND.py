...
class Perceptron:
    ...
    def fit(self, X, y, plot_interval=None):
        ...
        for epoch in range(self.n_epochs):
            ...
            # Removed periodic print to avoid clutter for potentially short NAND training
...
def plot_decision_boundary(X, y, weights, bias, epoch_num, ax, threshold=0.0, title_suffix=""):
    ...
    if 'Decision Boundary' in [l.get_label() for l in ax.lines]:
        ax.legend(loc='lower right') # Place boundary legend better for NAND
...
# === Example Usage for NAND ===

# 1. Define NAND function data
X_nand = np.array([
    [0, 0],
    [0, 1],
    [1, 0],
...
# 2. Create and Train the STEP FUNCTION Perceptron on NAND data
print("\n" + "="*30)
print("Training STEP FUNCTION Perceptron on NAND Gate")
print("="*30)
# Use fewer epochs as NAND usually converges quickly
ppn_nand = Perceptron(learning_rate=0.001, n_epochs=150, random_state=1)

# Train and store history (plot_interval=1 stores every epoch, needed for GIF)
ppn_nand.fit(X_nand, y_nand, plot_interval=1)

# 3. Save the learning process as a GIF
# Use a slightly longer duration per frame as there might be fewer frames
save_learning_gif(ppn_nand, X_nand, y_nand, filename="nand_perceptron_learning.gif", duration_per_frame=0.1)

# 4. Optional: Test final predictions
print("\n--- Final Predictions for NAND Gate ---")
for xi, target in zip(X_nand, y_nand):
    prediction = ppn_nand.predict(xi)
